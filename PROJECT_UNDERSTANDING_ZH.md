...
- 对话采样按轮记录 agent 输出，并在下一轮到来时，用“下一轮输入（next state）”评价上一轮
- next state 不仅可以是用户输入，也可以是环境反馈（执行结果、状态变化、工具返回等）
- PRM 根据 next state 给上一轮离散打分（`+1 / 0 / -1`），并可进行多次评估后多数投票（m-vote）
- 凑够 batch 后做 GRPO/PPO 风格更新；奖励会按 token 广播，并带有样本过滤逻辑
- 常见过滤：没有 next state 的末轮通常不参与 loss（仅保留 at-least-one 等兜底策略）
- 适合隐式反馈和环境结果明确的场景
...
进一步说，其本质是：  
**“人/环境提供反馈语义 + PRM 把语义映射为标量奖励”**，而不是人工逐条手工填写每条 reward。
...
5. 后台进行打分、样本构建与训练更新
6. 更新后参数会同步到服务端，后续对话逐步使用新模型（异步流水线下可能有短暂滞后）
7. 使用 `openclaw-test/` 或各子模块评测流程验证效果
